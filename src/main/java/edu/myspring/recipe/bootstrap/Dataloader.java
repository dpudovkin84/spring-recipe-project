package edu.myspring.recipe.bootstrap;

import edu.myspring.recipe.domain.*;
...
//        tacoNotes.setRecipe(tacosRecipe);
        ...
        Optional<UnitOfMeasure> optionalUnitOfMeasureOunce=unitOfMeasureRepository.findByDescription("Ounce");
        if(!optionalUnitOfMeasureOunce.isPresent()){
            throw new RuntimeException("Need set uom ounce");
        }
        UnitOfMeasure ounce=optionalUnitOfMeasureOunce.get();

        Recipe secretRecipe=new Recipe();
        secretRecipe.setDescription("My secrete recipe");
        secretRecipe.setDifficulty(Difficulty.MODERATE);
        secretRecipe.setCookTime(5);
        secretRecipe.setPrepTime(2);
        secretRecipe.addIngredient(new Ingredient("Big Mushroom",new BigDecimal(3.0),ounce));

        Notes secretNotes=new Notes();
        secretNotes.setRecipeNotes("Secret very sercet. Cook with caution, don't eat");

        secretRecipe.setNotes(secretNotes);
        secretRecipe.setDirections("Take every day morning");




        recipes.add(secretRecipe);
